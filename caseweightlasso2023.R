...
a = Sys.time()
ans = CD_all_concise(X,y,0.01)
b = Sys.time()
b-a

parallel::stopCluster(cl = my.cluster)

df<-read.csv('https://raw.githubusercontent.com/jbrownlee/Datasets/master/housing.csv')
X = centralize(as.matrix(df[,1:13]))
y = as.matrix(df[,14])

df<-read.table("prostate.txt")
# df[1,2] = 6
# df = cbind(df[,1]+rnorm(200,0,0.2),df)
X = centralize(as.matrix(df[,1:8]))
y = as.matrix(df[,9])

## 用block inverse的方法并不能让求逆变快
## 这个是测试如何combine foreach 并行运算得到的结果
doParallel::registerDoParallel(cl = my.cluster)
cfun <- function(a, b) rbind(a,b)
X = foreach(i=1:3, .combine='cfun') %do% matrix(nrow = 2, ncol = 3)
parallel::stopCluster(cl = my.cluster)
...
df<-read.table("prostate.txt")
# rs = sample(1:97,7)
X = centralize(as.matrix(df[rs,1:8]))
y = as.matrix(df[rs,9])

obj = lars(X,y,type='lasso')

beta_hat = as.vector(predict.lars(obj,mode='lambda', s=0, type = 'coefficients')$coefficients)

## parallel computing
## 如果直接跑foreach dopar 可以引用之前foreach之外的函数 但是如果foreach自己也在function里
## 就没有记忆了 得重新把外头的function再定义一遍
doParallel::registerDoParallel(cl = my.cluster)
aa = function(a) a+a
foreach (i = 1:3,.combine = 'c') %dopar% aa(i)
bb(3)
parallel::stopCluster(cl = my.cluster)



a = mvrnorm(5000,mu = c(0.1,0.1,0.1,0.1), Sigma = solve(t(X[,1:4])%*%X[,1:4]))
sample = diag(scale(a,c(0.1,0.1,0.1,0.1), F)%*%t(X[,1:4])%*%X[,1:4]%*%t(scale(a,c(0.1,0.1,0.1,0.1), F)))
sample = data.frame(x = sample)
ggplot(sample)+geom_density(aes(x=x))+ stat_function(fun = dchisq, args = list(df = 4),color = 'red1')

a = data.frame(a) %>% filter(lcavol>0,lweight>0,age>0, lbph>0)


x = 600:800/200
y = (1-pnorm(qnorm(0.95)-x))/pnorm(x)
y = (pnorm(qnorm(0.95)-x)-pnorm(-x))/pnorm(x)
plot(x,y)
...
x = -600:0/100
pnorm(-2-x) -0.95*pnorm(3-x)-0.05*pnorm(-3-x)
pnorm(-x-2) -0.05*pnorm(3-x)-0.95*pnorm(-3-x)


pnorm(3-x) + 0.05*pnorm(-3-x) - 0.05*pnorm(3-x)
x[556]
...
##### 2/18 ##### 
# 4.1 case influence graph mechanism
set.seed(1)
n = 10
root_x1 = rnorm(n,0,1)
root_x2 = rnorm(n,0,1)
...
y[10] = mean(y[-10])
p1 = CD_one_concise(X,y,10,0.001)
...
p2 = CD_one_concise(X,y,10,0.001)
...
y = X%*%c(4,1) + root_error
y[10] = 4
p3 = CD_one_concise(X,y,10,0.001)

# # scenario 4
# x1 = root_x1
# x1[10] = mean(root_x1[-10])+2
# x2 = root_x2
# x2[10] = mean(root_x2[-10])+2
# X = centralize(cbind(x1, x2))
# y = X%*%c(4,1) + root_error
# y[10]=-1
# p4 = CD_one_concise(X,y,10)
# 
# scenario 5
# x1 = root_x1
...
y = X%*%c(4,1) + root_error
y[10] = -4
p6 = CD_one_concise(X,y,10,0.001)


# X = cbind(c(0,0,2,-2,0,0,2,-2,-2),c(0,0,0,0,2,-2,2,2,-2))
# error = c(-2,0,2,2,-2,-2,2,2,-2)
# y = X%*%c(4,2) + error
# ex4.1 = CD_all_concise(X,y,0.01)
...
t1 = Sys.time()
newx = cbind(x,z)
ans = newx%*%solve(t(newx)%*%newx)%*%t(newx)
t2 = Sys.time()
v = (diag(n) - H)%*%z
t3 = Sys.time()
ans = H + v%*%t(v)/sum(v**2)
...
obj = lars(X,y,type='lasso')
as.vector(predict.lars(obj,mode='lambda', s=lambda, type = 'coefficients')$coefficients)


X2 = X[1:(n-1),]
y2 = y[1:(n-1)]
obj = lars(X2,y2,type='lasso')
as.vector(predict.lars(obj,mode='lambda', s=lambda, type = 'coefficients')$coefficients)
...
xdata<-t(scale(t(log10(xdata))))
ydata<-y1
result = CD_all_concise(xdata,ydata,plot = T,finesse = 0.02)

# wrong because of normalization
l = 0.000
X = centralize(X)
obj = lars(X,y,type='lasso',use.Gram = F)
v = as.vector(predict.lars(obj,mode='lambda', s=l, type = 'coefficients')$coefficients)
obj_1 = lars(X[-1,],y[-1],type='lasso',use.Gram = F)
v1 = as.vector(predict.lars(obj_1,mode='lambda', s=l, type = 'coefficients')$coefficients)
obj_2 = lars(X[-2,],y[-2],type='lasso',use.Gram = F)
v2 = as.vector(predict.lars(obj_2,mode='lambda', s=l, type = 'coefficients')$coefficients)

sum((X%*%v + mean(y) - X%*%v1 - mean(y[-1]))**2)
sum((X%*%v + mean(y) - X%*%v2 - mean(y[-2]))**2)
...
p = 100
n = 50
beta = c(5,4,3,2,1,rep(0,p))
x=matrix(rnorm(n*(p+5)),nrow=n)
# x[1,p/2] = x[1,p/2] + 2
y=x%*%beta + 2 + rnorm(n)
x = centralize(x)
# y[1] = y[1] + 2
# result = CD_all_concise(x,y,plot = T,finesse = 0.02,threshold = T)
# CD_all_concise(x[2:n,], y[2:n],plot = T,finesse = 0.02,threshold = T)
fit = cv.glmnet(x,y,standardize = F)
CD_one_fraction(x,y,fit$lambda.min*n)
